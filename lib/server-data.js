...
export async function createTaskRecord(task, currentUser, currentProfile) {
  const supabase = createSupabaseAdminClient();
  const ownerEmail = task.ownerEmail || currentUser.email;
  ...
  const payload = {
    ...task,
    owner: ownerProfile?.full_name || task.owner || currentProfile.full_name,
    ownerEmail,
...
export async function inviteMemberRecord(member, currentUser, currentProfile, redirectOrigin) {
  ...
  await supabase.from("activity_logs").insert({
    message: `Novo acesso criado para ${member.full_name}`,
  });
