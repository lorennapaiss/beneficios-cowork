...
async function seedIfNeeded(supabase) {
  ...
  // Se der erro na query (ex: Supabase acordando do pause), assume já seedado
  // para não bloquear o carregamento do workspace
  if (error) {
    _seeded = true;
    return;
  }
...
export async function getWorkspaceData(user) {
  ...
  const supabase = createSupabaseAdminClient();
  // seedIfNeeded é não-crítico: falha silenciosa para não derrubar o workspace
  const [profile] = await Promise.all([
    getProfile(user),
    seedIfNeeded(supabase).catch(() => {}),
  ]);
